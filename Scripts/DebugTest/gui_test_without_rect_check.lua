...
local function rawToUpperLevel(self)
    local pobjs = self.sceneinstance.objs
...
function objinstance:setUpperLayer(gobj)
    rawToUpperLevel(self)
end
...
local function isObjectIsUpper(obj1, obj2) --WTF!!
    ...
    if obj1.sceneinstance.isWindow and not obj2.sceneinstance.isWindow and not obj2.isWindow then
        print("1WTF", obj1.isText, obj1.isLabel, obj1.isWindow, obj1.button, obj1.isImage, obj1.text)
        print("1WTF2", obj2.isText, obj2.isLabel, obj2.isWindow, obj2.button, obj2.isImage, obj2.text)
        return true
    end

    if not obj1.sceneinstance.isWindow and obj2.sceneinstance.isWindow and not obj1.isWindow then
        print("2WTF", obj1.isText, obj1.isLabel, obj1.isWindow, obj1.button, obj1.isImage, obj1.text)
        print("2WTF2", obj2.isText, obj2.isLabel, obj2.isWindow, obj2.button, obj2.isImage, obj2.text)
        return false
    end
    ]]
...
local function updateTwoStep(self, updatedList)
    if not self:isVisible() then return end

    self.needUpdateTwoStep = true

    for _, obj in ipairs(self.sceneinstance.allObjs) do
        sc.yield()
        if self ~= obj and checkIntersection(self, obj) then
            if isObjectIsUpper(obj, self) then
                updateTwoStep(obj, updatedList)
            end
        end
    end
end
...
function objinstance:destroy()
    ...
    if self.isWindow then
        for _, lobj in ipairs(self.objs) do
            lobj:destroy()
        end
    end
...
function objinstance:clear(color)
    ...
    self.display.fillRect(self.x, self.y, getObjectWidth(self), getObjectHeight(self), color)
end
...
local function runCallbacks(self, ...)
...
function objinstance:_tick(click)
    ...
    if self.customHandler then
        if click == true and self.state then
            self:customHandler(-1, -1, "released", -1, "unknown") --release the pressed items when switching the scene
            self.state = false
        end
    elseif self.button then
        if click == true and not self.toggle then
            self.state = false
            runCallbacks(self, false, false)
        end
    ...
    if self.customHandler then
        if selected or (self.state and clktype == "released") then
            ...
            if self.handlerLocalPosition then
                tx, ty = tx - self.x, ty - self.y
            end
            if self:customHandler(tx, ty, clktype, btntype, nickname) then
                self:update()
            end
        end
    elseif self.button then
        if self.toggle then
            if selected and clktype == "pressed" then
                ...
                runCallbacks(self, self.state, true)
            end
        elseif selected and clktype == "pressed" then
            self.state = true
            runCallbacks(self, true, true)
        elseif clktype == "released" and self.state then
            self.state = false
            runCallbacks(self, false, selected)
        end
    ...
    elseif self.isWindow then
        ...
        if elementCapture and clktype == "pressed" then
            ...
            if toUpperLevel(self) then
                self.guiinstance.needFlushFlag = true
                self.rootsceneinstance.needUpdate = true
            end
...
function sceneinstance:_tick(clean)
    if not clean and self.onTick then
        self:onTick()
    end
    ...
    for i = #self.objs, 1, -1 do
        ...
        sc.yield()
        local objectUsed = obj:_tick(click)
        otherUsed = otherUsed or objectUsed
        if objectUsed and click ~= true then
    ...
    if click ~= true then
        if self.bgHandle and not otherUsed and click then
            self:bgHandle(click)
        end
        return click
    end
...
local function initObject(self, obj)
    ...
    obj.setUpperLayer = objinstance.setUpperLayer
