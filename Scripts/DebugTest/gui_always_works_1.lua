...
function objinstance:_tick(click)
    ...
    if not self.universalCallbacksState then
        return false
    end

    if returnOverride ~= nil then
...
function sceneinstance:_tick(clean)
    ...
    local userClick = click
    if userClick == true then userClick = nil end
    for i = #self.allObjs, 1, -1 do
        sc.yield()
        ...
        if obj.isWindow and not obj.up_hide and obj.onTickIfOpen then
            ...
            if visible then
                obj:onTickIfOpen(userClick)
            end
        end

        if obj.universalCallbacksState and obj.onTickIfPressed then
            obj:onTickIfPressed(userClick, getLocalPosition(obj, click))
        end

        if userClick and obj.handlerAllClicks and obj.customHandler then
            local tx, ty = userClick[1], userClick[2]
            local lx, ly = tx - obj.x, ty - obj.y
            local selected = userClick[1] >= obj.x and userClick[2] >= obj.y and userClick[1] < (obj.x + obj.sizeX) and userClick[2] < (obj.y + obj.sizeY)
            local clktype = userClick[3]
            local btntype = userClick[4]
            local nickname = userClick[5]

            if obj.handlerLocalPosition then
                tx, ty = lx, ly
            end

            if obj:customHandler(tx, ty, clktype, btntype, nickname, selected) then
                obj:update()
            end
        end
    end
