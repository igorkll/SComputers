...
function callback_loop()
    ...
    --this part of the API is only accessible by a holographic display
    local tick = getTick()
    local sinVal = math.rad(tick * 8)
    display.setHoloScale(1 + (math.sin(sinVal) * 0.2), 1 + (math.cos(sinVal) * 0.2))
    display.setHoloRotation(0, math.rad(tick * 0.5), 0)
    display.setHoloOffset(0, math.sin(math.rad(tick)) * 0.2, 0)
end